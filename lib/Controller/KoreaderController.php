...
class KoreaderController extends Controller {
    ...
    private function saveDocumentProgress($userId, $document, $progress, $percentage, $device, $deviceId) {
        // Shared with the web reader's own save path, so the two cannot disagree
        // about what fits in these columns. Every value here is device-controlled,
        // and anything too long produced a 500 rather than a stored position.
        $percentage = $this->readingProgress->normalizePercentage($percentage);
        $device = $this->readingProgress->normalizeDevice($device);
        $deviceId = $this->readingProgress->normalizeDevice($deviceId);
